...
Usage:
openmmMD_two_min.py [--inpdb=<pdb>] [--pH=<pH>] [--no_restraints] 
...
def setup_system(modeller, forcefield, solvmol: str, no_restraints: bool):
    ...
    #y = 130
    #z = 130
    #xvec = mm.Vec3(13.32, 0.0, 0.0)
    #yvec = mm.Vec3(0.0, 13.32, 0.0)
    #zvec = mm.Vec3(0.0, 0.0, 13.32)
...
def main():
    arguments = docopt(__doc__, version='openmmMD_two_min.py')
    pdb_clean = clean_pdb(arguments['--inpdb'], arguments['--pH'])
    pdb = pdb_clean[0]
    stem = pdb_clean[1]
    pdbout = str(stem + "vac_min.pdb")
    min_vac = vacuum_minimization(pdb, pdbout)
    pdb = app.PDBFile(min_vac)
    ...
    for j in range(1,2):
        ...
        sim_run = md_nvt(simulation, csvname, 500000, 1000, pdbname, integrator, system)
